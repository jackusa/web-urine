...
//        "url must end with '.html'");
  ...
  protected void onSessionInvalid(HttpServletRequest request,
    ...
    String targetUrl;
    // todo 不能用url后缀简单的判断是否来自浏览器 StringUtils.endsWithIgnoreCase(sourceUrl, ".html")
    if (LoginResponseType.REDIRECT.equals(securityProperties.getBrowser().getSignInResponseType())) {
      // 如果请求的URL来自登录或者注册还是返回登录、注册的URL，否则返回 默认设置的session失效URL
